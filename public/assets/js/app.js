...
(function ($) {
    ...
    function initLeftSidebar() {
        var self = this;
         /* 사이드바 초기값으로 일단 class 전부 삭제 시키고 시작하기 */
         $(".side-nav-item").removeClass("menuitem-active");
         $(".side-nav-item li").removeClass("menuitem-active");
         $(".collapse").removeClass("show");
         $(".side-nav-item .side-nav-link").removeClass("collapsed");
         $(".side-nav-item .side-nav-link").attr("aria-expanded", "false"); 
         /* 사이드바 초기값으로 일단 class 전부 삭제 시키고 시작하기 END */
    ...
    /* 2024.01.18 - Saylee  
     * initLeftSidebar 함수는 리액트용이 아니기때문에 페이지 새로고침 될때만 해당 url과 동일한 id를 가지는 메뉴를  active 하게끔 스크립트가 짜여져있음 
     * 그러므로 아이템을 클릭할때 active메뉴 선택하도록 함수호출해주기
     */
    $(document).on("click", $(".side-nav-item"), function (e) {
        initLeftSidebar()
    })
